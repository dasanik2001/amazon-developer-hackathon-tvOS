...
export const DEFAULT_SERVER_URL = process.env.EXPO_PUBLIC_API_URL || 'http://192.168.0.4:3001';

/**
 * Detect host IP dynamically from Metro bundler's scriptURL in Expo Go / React Native.
 * If Metro is loading from "http://192.168.0.4:8081/index.bundle...", this extracts the IP.
 */
...
export async function getBaseUrl(): Promise<string> {
  try {
    ...
    if (custom && custom.trim()) {
      // Auto-migrate stale hardcoded addresses if previously stored in AsyncStorage
      if (custom.includes('192.168.0.102') || custom.includes('192.168.0.100')) {
        const updated = custom.replace(/192\.168\.0\.(102|100)/, '192.168.0.4');
  ...
  return 'http://192.168.0.4:3001';
}

/** Set a custom server host (e.g. http://192.168.0.4:3001 or https://my-tunnel.ngrok.io) */
...
export const pairingApi = {
  ...
  confirmPairingSession: (sessionId?: string, pairingCode?: string, email?: string, userId?: string) =>
    request<any>('/api/pairing/confirm', {
      method: 'POST',
      body: JSON.stringify({ sessionId, pairingCode, email, userId }),
    }),
